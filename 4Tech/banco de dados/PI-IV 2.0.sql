...
insert into usuarios_clientes(email,senha,cliente_id) values ('joao@gmail.com',md5('12345678'),@cliente_id);
commit;

select*from clientes;
select*from enderecos_de_cobranca;
select*from enderecos_de_entrega;
...
CREATE TABLE pedidos (
    ...
    status ENUM('em andamento', 'concluído', 'cancelado','aguardando pagamento') NOT NULL,
    cliente_id int(10) NOT NULL,
    produto_id int(10) NOT NULL
);

alter table pedidos add constraint fk_pedidos_clientes foreign key(cliente_id) references clientes(id);
alter table pedidos add constraint fk_pedidos_produtos foreign key(produto_id) references produto(id);
...
alter table pedidos_clientes_produtos add constraint fk_pedidos_clientes_produtos_clientes foreign key(cliente_id) references clientes(id);
alter table pedidos_clientes_produtos add constraint fkpedidos_clientes_produtos_produtos foreign key(produto_id) references produto(id);
alter table pedidos_clientes_produtos add constraint fkpedidos_clientes_produtos_pedidos foreign key(pedido_id) references pedidos(id);